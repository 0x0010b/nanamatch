...
class LoginUsernameViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if !Util.connectedToNetwork() {
            self.present(AppMessage.messagePopup(title: "Internet connection", message: "There is no internet connection, try reconnecting to Wi-Fi."), animated: true, completion: nil)
        }
    ...
    @IBAction func btnSiguiente(_ sender: Any) {
        ...
        if validateIsCorrect(username) {
            performSegue(withIdentifier: "LoginPasswordViewController", sender: sender)
        } else {
            // Shake
            self.shake()
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "LoginPasswordViewController" {
            let controller = segue.destination as! LoginPasswordViewController
